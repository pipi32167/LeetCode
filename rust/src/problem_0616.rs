...
impl Solution {
  fn is_match(s: &String, start: usize, word: &String) -> bool {
    if word.len() > s.len() - start {
      return false;
    }

    let b1 = s.as_bytes();
    let b2 = word.as_bytes();
    for i in 0..word.len() {
      if b1[start + i] != b2[i] {
        return false;
      }
  ...
  fn match_all(s: &String, word: &String) -> Vec<usize> {
    let mut ret = vec![];
    if word.len() > s.len() {
      return ret;
    }
    for i in 0..=s.len() - word.len() {
      if Self::is_match(s, i, word) {
        ret.push(i);
      }
    }
    ret
  ...
  pub fn add_bold_tag(s: String, dict: Vec<String>) -> String {
    ...
    let mut bolds = vec![false; s.len()];
    for word in dict {
      let matches = Self::match_all(&s, &word);
      for m in matches {
        for i in m..m+word.len() {
          bolds[i] = true;
        }
      }
    }
